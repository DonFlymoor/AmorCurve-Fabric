...
public class CurveConfig implements ConfigData {
    ...
    public void validatePostLoad() throws ValidationException {
        ArmorCurveMod.formulae[0] = new Expression(armor);
        try {
            ArmorCurveMod.formulae[0].with("damage", "1").and("armor", "10").and("toughness", "0").eval();
        } catch (Expression.ExpressionException e) {
            throw new ValidationException("invalid formula " + armor);
        }
        ArmorCurveMod.formulae[1] = new Expression(armorToughness);
        try {
            ArmorCurveMod.formulae[1].with("damage", "1").and("armor", "10").and("toughness", "0").eval();
        } catch (Expression.ExpressionException e) {
            throw new ValidationException("invalid formula " + armorToughness);
        }
        ArmorCurveMod.formulae[2] = new Expression(enchantments);
        try {
            ArmorCurveMod.formulae[2].with("damage", "1").and("enchant", "2").eval();
        } catch (Expression.ExpressionException e) {
            throw new ValidationException("invalid formula " + enchantments);
        }
        ArmorCurveMod.formulae[3] = new Expression(degradation);
        try {
            ArmorCurveMod.formulae[3].with("remaining", "100").and("max", "100").eval();
        } catch (Expression.ExpressionException e) {
